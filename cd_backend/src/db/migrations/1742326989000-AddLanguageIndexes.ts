...
export class AddLanguageIndexes1742326989000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_name_language_name" ON "name" ("language_id", "name") WHERE "narrative_id" IS NOT NULL`
    );

    // Add index for descriptions by language for faster lookup
    await queryRunner.query(
      `CREATE INDEX "idx_description_language" ON "description" ("language_id") WHERE "narrative_id" IS NOT NULL`
    );

    // Add index for narratives by creation date for efficient sorting
    await queryRunner.query(`CREATE INDEX "idx_narrative_created_at" ON "narrative" ("created_at")`);

    // Add index on narrative id in the name table for faster joins
    await queryRunner.query(
      `CREATE INDEX "idx_name_narrative" ON "name" ("narrative_id") WHERE "narrative_id" IS NOT NULL`
    );

    // Add index on narrative id in the description table for faster joins
    await queryRunner.query(`CREATE INDEX "idx_description_narrative" ON "description" ("narrative_id")`);
  }
