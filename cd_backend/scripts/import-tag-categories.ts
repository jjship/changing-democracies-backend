...
import { TagCategoryEntity } from '../src/db/entities/TagCategory';
...
// Required languages for tag category import with correct ISO codes
...
// Process and import tag categories from CSV
async function importTagCategoriesFromCsv(connection: DataSource, languageMap: Map<string, LanguageEntity>) {
  console.log('\n📥 Importing tag categories from CSV...');

  // Path to CSV file (in the tag_categories_migration folder)
  const csvFilePath = path.join(process.cwd(), 'tag_categories_migration', 'categories.csv');
  ...
  console.log(`Found ${records.length} tag categories to import`);

  // Get repositories
  const tagCategoryRepository = connection.getRepository(TagCategoryEntity);
  ...
  try {
    ...
    // Process each tag category (row in CSV)
    for (const record of records) {
      ...
      await queryRunner.manager.save(tagCategory);

      console.log(`Created tag category with ID: ${tagCategory.id}`);
      ...
      for (const [langHeader, langCode] of Object.entries(languageHeaderToCode)) {
        const categoryName = record[langHeader];
      ...
      if (names.length > 0) {
        ...
        console.log(`Added ${names.length} translations for tag category ${tagCategory.id}`);
        totalNames += names.length;
        importedCategories++;
      } else {
        // If no translations were created, remove the tag category
        await queryRunner.manager.remove(tagCategory);
        console.warn(`⚠️ No valid translations found for tag category, removing empty category`);
        skippedCategories++;
      }
    }
    ...
    console.log('\n📊 Import summary:');
    console.log(`✅ Successfully imported ${importedCategories} tag categories with ${totalNames} translations`);
    console.log(`⚠️ Skipped ${skippedCategories} tag categories`);
  } catch (error) {
...
// Main function
async function main() {
  console.log('🚀 Starting tag category import process');
  ...
  try {
    ...
    await importTagCategoriesFromCsv(connection, languageMap);

    console.log('\n✅ Tag category import completed successfully');
  } catch (error: any) {
    console.error('\n❌ Tag category import failed:', error.message || String(error));
    process.exit(1);
  } finally {
    // Close connection
    if (connection) {
